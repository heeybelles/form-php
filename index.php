...
$quiz = [
    ...
    "Matemática" => [
        ['pergunta'=>"1. Qual é o valor de π (pi) arredondado para duas casas decimais?", 
...
if(isset($_POST['resposta']) && isset($_POST['tema'])){
    ...
    foreach($quiz[$tema] as $i => $questao){
        if(isset($respostasUsuario[$i]) && $respostasUsuario[$i] == $questao['resposta']){
            $acertos++;
        }
    }
    $pontuacao = ($acertos / 5) * 10;
    if($pontuacao == 10){
    ...
    echo "<fieldset class='resultado'>";
    echo "<legend>Resultado - $tema</legend>";
    echo "<h3>Você acertou $acertos de 5 perguntas.</h3>";
    echo "<h3>Sua pontuação: $pontuacao / 10</h3>";
    echo "<p><b>$mensagem</b></p>";
    echo '<form method="post"><input type="submit" class="botao-refazer" value="Jogar Novamente"></form>';
    echo "</fieldset>";
